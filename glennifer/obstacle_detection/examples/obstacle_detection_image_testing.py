...
while True:
    ...
    image, contours, hierarchy = cv2.findContours(
        unknown, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
        ...
        try:
            ...
            if(equi_diameter > LOW_DIAMETER_BOUND and equi_diameter < HIGH_DIAMETER_BOUND):
                ...
                img_fg = cv2.bitwise_and(
                    depth_frame.asarray(), depth_frame.asarray(), mask=mask)
                ...
                coords = registration.getPointXYZ(
                    depth_frame, max_loc[0], max_loc[1])
                ...
                cv2.putText(img, str(max_val), (cx, cy + 25), font,
                            0.4, (255, 255, 255), 1, cv2.LINE_AA)
                cv2.putText(img, str(mm_diameter), (cx, cy + 50),
